...
RadioStation::BookmarksWalker::BookmarksWalker(RadioStation& radiostation, Gtk::Menu* menu)
...
bool
RadioStation::BookmarksWalker::for_each(pugi::xml_node& node)
{
    auto name = node.name();
    ...
    if (is_group) {
        ...
        if (strncasecmp(station_name, kSeparatorPrefix, kSeparatorPrefixLength) == 0) {
            ...
            sub_item->signal_activate().connect(sigc::bind<Glib::ustring, Glib::ustring, Glib::ustring>(
                sigc::mem_fun(radiostation, &RadioStation::on_station_button), station_group_name, station_name, station_url));
...
bool
RadioStation::init(int argc, char** argv, std::shared_ptr<CmdLineOptions>& opts)
{
    ...
    // app = Gtk::Application::create(argc, argv, "github.com/sparkylinux/radiostation");
    app = Gtk::Application::create("github.com/sparkylinux/radiostation");
    ...
    em->state_changed.connect(sigc::mem_fun(*this, &RadioStation::on_station_changed_signal));
    em->broadcast_info_changed.connect(sigc::mem_fun(*this, &RadioStation::on_broadcast_info_changed_signal));
...
RadioStation::run()
{
    ...
    sigc::slot<bool> resume_slot = sigc::bind(sigc::mem_fun(*this, &RadioStation::resume), cmd_line_options->resume);
...
void
RadioStation::on_station_button(Glib::ustring group_name, Glib::ustring station_name, Glib::ustring station_url)
{
...
void
RadioStation::on_current_station_button()
{
...
bool
RadioStation::resume(bool resume_last_station)
{
...
RadioStation::build_menu()
{
    ...
    menu_item->signal_activate().connect(sigc::mem_fun(*this, &RadioStation::on_reload_button));
    menu->append(*menu_item);

    name = "About";
    menu_item = Gtk::manage(new Gtk::MenuItem(name));
    menu_item->signal_activate().connect(sigc::mem_fun(*this, &RadioStation::on_about_button));
    menu->append(*menu_item);

    name = "Quit";
    menu_item = Gtk::manage(new Gtk::MenuItem(name));
    menu_item->signal_activate().connect(sigc::mem_fun(*this, &RadioStation::on_quit_button));
    menu->append(*menu_item);
...
bool
RadioStation::parse_bookmarks_file()
{
    bool status = false;
...
void
RadioStation::load_configuration()
{
...
void
RadioStation::set_current_station(bool turn_on)
{
    ...
    if (player->has_station()) {
        ...
        if (current_station_menu_entry == nullptr) {
            ...
            current_station_menu_entry->signal_activate().connect(sigc::mem_fun(*this, &RadioStation::on_current_station_button));
...
void
RadioStation::set_current_broadcast(Glib::ustring info)
{
...
void
RadioStation::on_station_changed_signal(Glib::ustring station, StationState state)
{
    if (state == em->state) {
        return;
    }
...
void
RadioStation::on_broadcast_info_changed_signal(Glib::ustring /*station*/, Glib::ustring info)
{
...
void
RadioStation::copy_default_bookmarks(std::string src_file)
{
    auto home = getenv("HOME");
    if (home == nullptr) {
        return;
...
bool
RadioStation::file_exists(const std::string& dir, const std::string& file)
{
...
bool
RadioStation::dir_exists(const std::string& dir)
{
    if (dir.empty()) {
        return false;
    }
